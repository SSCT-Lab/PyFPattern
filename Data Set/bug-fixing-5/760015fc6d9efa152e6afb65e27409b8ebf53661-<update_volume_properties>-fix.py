def update_volume_properties(self):
    ...
    try:
        (rc, resp) = request((self.api_url + ('/storage-systems/%s/%s/%s/' % (self.ssid, self.volume_resource_name, self.volume_detail['id']))), data=json.dumps(update_volume_req), headers=HEADERS, method='POST', url_username=self.api_usr, url_password=self.api_pwd, validate_certs=self.validate_certs, timeout=120)
    except Exception:
        err = get_exception()
