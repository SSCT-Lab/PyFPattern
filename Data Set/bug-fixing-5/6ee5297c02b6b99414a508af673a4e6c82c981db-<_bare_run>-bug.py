def _bare_run(self, cmd, in_data, sudoable=True, checkrc=True):
    ...
    display_cmd = ' '.join((shlex_quote(to_text(c)) for c in cmd))
    ...
    if ((to_bytes(self.get_option('ssh_executable')) in cmd) and sudoable):
        ...
        if prompt:
            ...
            display.debug(('Initial state: %s: %s' % (states[state], to_text(prompt))))
        elif (self._play_context.become and self.become.success):
            state = states.index('awaiting_escalation')
            display.debug(('Initial state: %s: %s' % (states[state], to_text(self.become.success))))
    b_stdout = b_stderr = b''
