def exec_module(self, **kwargs):
    ...
    if ((self.state == 'present') and changed):
        frontend_ip_configurations_param = ([self.network_models.FrontendIPConfiguration(name=item.get('name'), public_ip_address=self.get_public_ip_address(item.get('public_ip_address'))) for item in self.frontend_ip_configurations] if self.frontend_ip_configurations else None)
        ...
        param = self.network_models.LoadBalancer(location=self.location, frontend_ip_configurations=frontend_ip_configurations_param, backend_address_pools=backend_address_pools_param, probes=probes_param, inbound_nat_pools=inbound_nat_pools_param, load_balancing_rules=load_balancing_rules_param)
