    ...
    for rule in service.rules:
        if ((old_rule.rule_type == rule.rule_type) and (old_rule.rule_control == rule.rule_control) and (old_rule.rule_module_path == rule.rule_module_path)):
            if (index == 0):
                ...
                changed = True
            elif ((new_rule.rule_type != service.rules[(index - 1)].rule_type) or (new_rule.rule_control != service.rules[(index - 1)].rule_control) or (new_rule.rule_module_path != service.rules[(index - 1)].rule_module_path) or (new_rule.rule_module_args != service.rules[(index - 1)].rule_module_args)):
