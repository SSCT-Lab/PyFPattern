    ...
    if ((len(sys.argv) == 1) or ((len(sys.argv) >= 2) and (('--help' in sys.argv[1:]) or (sys.argv[1] in ('--help-commands', 'egg_info', '--version', 'clean'))))):
        ...
        if (scipy_status['up_to_date'] is False):
            if scipy_status['version']:
                raise ImportError('Your installation of Scientific Python (SciPy) {0} is out-of-date.\n{1}{2}'.format(scipy_status['version'], scipy_req_str, instructions))
            else:
