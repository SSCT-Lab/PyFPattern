    ...
    if (secure in ('never', 'try', 'starttls')):
        try:
            ...
            if (secure == 'try'):
                ...
                module.fail_json(rc=1, msg=('Unable to Connect to %s:%s: %s' % (host, port, to_native(e))), exception=traceback.format_exc())
    if (secure == 'always'):
        try:
            smtp = smtplib.SMTP_SSL(timeout=timeout)
            (code, smtpmessage) = smtp.connect(host, port=port)
    ...
    if (attach_files is not None):
        for file in attach_files.split():
            try:
                ...
                module.fail_json(rc=1, msg=("Failed to send mail: can't attach file %s: %s" % (file, to_native(e))), exception=traceback.format_exc())
    composed = msg.as_string()
    try:
        smtp.sendmail(sender_addr, set(addr_list), composed)
    except Exception as e:
        module.fail_json(rc=1, msg=('Failed to send mail to %s: %s' % (', '.join(addr_list), to_native(e))), exception=traceback.format_exc())
    smtp.quit()
