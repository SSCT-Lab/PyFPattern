def create_volume(self, pool_id, name, size_unit, size, segment_size_kb, data_assurance_enabled):
    ...
    try:
        (rc, resp) = request((self.api_url + ('/storage-systems/%s/volumes' % self.ssid)), data=json.dumps(volume_add_req), headers=self._post_headers, method='POST', url_username=self.api_usr, url_password=self.api_pwd, validate_certs=self.validate_certs, timeout=120)
    except Exception:
        err = get_exception()
