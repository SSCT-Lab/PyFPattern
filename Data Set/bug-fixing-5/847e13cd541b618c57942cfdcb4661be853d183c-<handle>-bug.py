def handle(self, **options: Any) -> None:
    ...
    if ((not options['agree_to_terms_of_service']) and (not options['rotate_key'])):
        raise CommandError('You must agree to the Zulipchat Terms of Service: https://zulipchat.com/terms/.  Run as:\n  python manage.py register_remote_server --agree_to_terms_of_service\n')
    ...
    print('')
    if ((not options['agree_to_terms_of_service']) and (not options['rotate_key'])):
        raise CommandError('You must agree to the Terms of Service: https://zulipchat.com/terms/\n  python manage.py register_remote_server --agree_to_terms_of_service\n')
    ...
    try:
        ...
        raise CommandError(('Network error connecting to push notifications service (%s)' % (settings.PUSH_NOTIFICATION_BOUNCER_URL,)))
    try:
