def main():
    ...
    module = AnsibleModule(argument_spec=argument_spec, supports_check_mode=True, required_together=[['device_id', 'private_ip_address']])
    if (not HAS_BOTO):
    ...
    try:
        ...
        if (state == 'present'):
            if device_id:
                result = ensure_present(ec2, module, domain, address, private_ip_address, device_id, reuse_existing_ip_allowed, allow_reassociation, module.check_mode, isinstance=is_instance)
            else:
                if address:
                    changed = False
                else:
