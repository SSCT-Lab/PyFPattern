def main():
    module = AnsibleModule(argument_spec=dict(src=dict(type='path'), original_basename=dict(type='str'), content=dict(type='str', no_log=True), dest=dict(type='path', required=True), backup=dict(type='bool', default=False), force=dict(type='bool', default=True, aliases=['thirsty']), validate=dict(type='str'), directory_mode=dict(type='raw'), remote_src=dict(type='bool'), local_follow=dict(type='bool'), checksum=dict()), add_file_common_args=True, supports_check_mode=True)
