def forward(fn, input, hx, weight, output, hy):
    with torch.cuda.device_of(input):
        ...
        if (fn.input_size != input.size(2)):
            raise RuntimeError('input.size(2) must be equal to input_size. Expected {}, got {}'.format(fn.input_size, input.size(2)))
        ...
        if ((desc_name not in fn.dropout_state) or (fn.dropout_state[desc_name].get() is None)):
            fn.dropout_state[desc_name] = Unserializable(init_dropout_descriptor(fn, handle))
