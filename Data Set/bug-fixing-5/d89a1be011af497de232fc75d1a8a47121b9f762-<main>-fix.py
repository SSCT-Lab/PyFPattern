def main():
    ...
    element_spec = dict(name=dict(), mode=dict(choices=['access', 'trunk']), access_vlan=dict(), native_vlan=dict(type='int'), trunk_vlans=dict(type='list'), unit=dict(default=0, type='int'), description=dict(), enhanced_layer=dict(default=True, type='bool'), state=dict(default='present', choices=['present', 'absent']), active=dict(default=True, type='bool'))
    ...
    for param in params:
        ...
        param_to_xpath_map['mode']['xpath'] = ('interface-mode' if param['enhanced_layer'] else 'port-mode')
