def post(self, request):
    ...
    try:
        group = Group.objects.get(project__in=Project.objects.filter(organization__in=integration.organizations.all()), id=group_id)
    except Group.DoesNotExist:
        logger.error('slack.action.invalid-issue', extra=logging_data)
        return self.respond(status=403)
    try:
        identity = Identity.objects.get(external_id=user_id, idp=IdentityProvider.objects.get(organization=group.organization))
