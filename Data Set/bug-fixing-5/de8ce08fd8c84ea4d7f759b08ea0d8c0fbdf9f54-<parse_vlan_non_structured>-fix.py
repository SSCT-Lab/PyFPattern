    ...
    for vlan in vlans:
        ...
        if vlan_match:
            ...
            if name_match:
                ...
                obj['name'] = name
                state_match = re.search('{0}\\s*{1}\\s*(\\S+)'.format(vlan_id, re.escape(name)), vlan, re.M)
                if state_match:
                    ...
                    interfaces = list()
                    intfs_match = re.search('{0}\\s*{1}\\s*{2}\\s*(.*)'.format(vlan_id, re.escape(name), vlan_state_match), vlan, re.M)
                    if intfs_match:
