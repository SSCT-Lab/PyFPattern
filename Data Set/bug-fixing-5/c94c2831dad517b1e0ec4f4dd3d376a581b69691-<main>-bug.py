def main():
    ...
    if (module.params['state'] == 'present'):
                ...
                for (key, value) in enumerate(tag_array):
                    if (value['Key'] == 'Name'):
                        ...
                        if (current_name != name):
                            results['name'] = gw_mgr.tag_cgw_name(results['gateway']['CustomerGateway'][0]['CustomerGatewayId'], module.params['name'])
        ...
        if existing['CustomerGateway']:
            results['gateway'] = existing
            if (not module.check_mode):
                results['gateway'] = gw_mgr.ensure_cgw_absent(existing['CustomerGateway'][0]['CustomerGatewayId'])
            results['changed'] = True
