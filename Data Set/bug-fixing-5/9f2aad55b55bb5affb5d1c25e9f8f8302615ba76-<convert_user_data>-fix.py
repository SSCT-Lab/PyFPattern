def convert_user_data(raw_data: List[ZerverFieldsT], realm_id: int) -> List[ZerverFieldsT]:
    ...
    def process(in_dict: ZerverFieldsT) -> ZerverFieldsT:
        ...
        if in_dict.get('avatar'):
            ...
            avatar_source = 'G'
        return build_user(avatar_source=avatar_source, date_joined=date_joined, delivery_email=delivery_email, email=email, full_name=full_name, id=id, is_active=is_active, is_realm_admin=is_realm_admin, is_guest=is_guest, realm_id=realm_id, short_name=short_name, timezone=timezone)
