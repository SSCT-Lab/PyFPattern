def main():
    ...
    try:
        ...
        if (state == 'present'):
            ret = hosts_module.create(deploy_hosted_engine=((module.params.get('hosted_engine') == 'deploy') if (module.params.get('hosted_engine') is not None) else None), result_state=(hoststate.UP if (host is None) else None), fail_condition=(hosts_module.failed_state_after_reinstall if (host is None) else (lambda h: False)))
            ...
            if module.params['check_upgrade']:
                hosts_module.action(action='upgrade_check', action_condition=(lambda host: (not host.update_available)), wait_condition=(lambda host: (host.update_available or (len([event for event in events_service.list(from_=int(last_event.id), search='type=885') if (host.name in event.description)]) > 0))), fail_condition=(lambda host: (len([event for event in events_service.list(from_=int(last_event.id), search=('type=839 or type=887 and host.name=%s' % host.name))]) > 0)))
                hosts_module._changed = False
            ret = hosts_module.action(action='upgrade', action_condition=(lambda h: h.update_available), wait_condition=(lambda h: (h.status == result_state)), post_action=(lambda h: time.sleep(module.params['poll_interval'])), fail_condition=(lambda h: (hosts_module.failed_state_after_reinstall(h) or (len([event for event in events_service.list(from_=int(last_event.id), search='type=65 or type=140')]) > 0))), reboot=module.params['reboot_after_upgrade'])
        elif (state == 'iscsidiscover'):
            host_id = get_id_by_name(hosts_service, module.params['name'])
            iscsi_param = module.params['iscsi']
            iscsi_targets = hosts_service.service(host_id).iscsi_discover(iscsi=otypes.IscsiDetails(port=int(iscsi_param.get('port', 3260)), username=iscsi_param.get('username'), password=iscsi_param.get('password'), address=iscsi_param.get('address')))
            ret = {
                'changed': False,
                'id': host_id,
            ...
            host_id = get_id_by_name(hosts_service, module.params['name'])
            iscsi_param = module.params['iscsi']
            ret = hosts_module.action(action='iscsi_login', iscsi=otypes.IscsiDetails(port=int(iscsi_param.get('port', 3260)), username=iscsi_param.get('username'), password=iscsi_param.get('password'), address=iscsi_param.get('address'), target=iscsi_param.get('target')))
        elif (state == 'started'):
            ret = hosts_module.action(action='fence', action_condition=(lambda h: (h.status == hoststate.DOWN)), wait_condition=(lambda h: (h.status in [hoststate.UP, hoststate.MAINTENANCE])), fail_condition=hosts_module.failed_state_after_reinstall, fence_type='start')
        elif (state == 'stopped'):
            ...
            ret = hosts_module.action(action='fence', wait_condition=(lambda h: (h.status == hoststate.UP)), fail_condition=hosts_module.failed_state_after_reinstall, fence_type='restart')
