def main():
    ...
    argument_spec.update(url=dict(required=True), dest=dict(required=True, type='path'), backup=dict(default=False, type='bool'), sha256sum=dict(default=''), checksum=dict(default=''), timeout=dict(required=False, type='int', default=10), headers=dict(required=False, default=None), tmp_dest=dict(required=False, default='', type='path'))
    module = AnsibleModule(argument_spec=argument_spec, add_file_common_args=True)
    ...
    try:
        ...
        md5sum = None
    res_args = dict(url=url, dest=dest, src=tmpsrc, md5sum=md5sum, checksum_src=checksum_src, checksum_dest=checksum_dest, changed=changed, msg=info.get('msg', ''))
    if backup_file:
