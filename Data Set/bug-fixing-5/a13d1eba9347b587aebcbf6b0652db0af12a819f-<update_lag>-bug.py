def update_lag(client, lag_id, lag_name, min_links, num_connections, wait, wait_timeout):
    ...
    if (min_links and (min_links > num_connections)):
        raise DirectConnectError(msg='The number of connections {0} must be greater than the minimum number of links {1} to update the LAG {2}'.format(num_connections, min_links, lag_id), last_traceback=None, response=None)
    while True:
        try:
