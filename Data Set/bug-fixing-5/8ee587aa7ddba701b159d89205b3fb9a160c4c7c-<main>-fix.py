def main():
    ...
    try:
        ...
        if (state in ('present', 'running', 'next_run')):
            ...
            ret = vms_module.create(entity=vm, result_state=(otypes.VmStatus.DOWN if (vm is None) else None), update_params=({
                'next_run': module.params['next_run'],
            } if (module.params['next_run'] is not None) else None), clone=module.params['clone'], clone_permissions=module.params['clone_permissions'], _wait=(True if ((not module.params['wait']) and (state == 'running')) else module.params['wait']))
