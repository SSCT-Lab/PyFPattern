def plot_network(symbol, title='plot', save_format='pdf', shape=None, node_attrs={
    ...
    for node in nodes:
        ...
        if (op == 'null'):
            ...
            label = ('Convolution\\n%s/%s, %s' % ('x'.join(_str2tuple(node['attr']['kernel'])), ('x'.join(_str2tuple(node['attr']['stride'])) if ('stride' in node['attr']) else '1'), node['attr']['num_filter']))
            attr['fillcolor'] = cm[1]
            ...
            label = ('Pooling\\n%s, %s/%s' % (node['attr']['pool_type'], 'x'.join(_str2tuple(node['attr']['kernel'])), ('x'.join(_str2tuple(node['attr']['stride'])) if ('stride' in node['attr']) else '1')))
            attr['fillcolor'] = cm[4]
