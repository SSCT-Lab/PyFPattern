def _make_atomic_symbol_function(handle, name):
    ...
    doc_str = _build_doc(func_name, py_str(desc.value), arg_names, arg_types, [py_str(arg_descs[i]) for i in range(narg)], key_var_num_args, ret_type)
    dtype_name = None
    arr_name = None
    ndsignature = []
    signature = []
    ...
    if arr_name:
        ...
        if (dtype_name is not None):
            code.append(("\n    if '%s' in kwargs:\n        kwargs['%s'] = _numpy.dtype(kwargs['%s']).name" % (dtype_name, dtype_name, dtype_name)))
