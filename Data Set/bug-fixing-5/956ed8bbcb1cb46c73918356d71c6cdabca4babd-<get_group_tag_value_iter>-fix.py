def get_group_tag_value_iter(self, project_id, group_id, environment_id, key, callbacks=()):
    ...
    results = snuba.query(start=start, end=end, groupby=['tags_value'], filter_keys={
        ...
        'tags_key': [key],
        'issue': [group_id],
    }, aggregations=[['count()', '', 'times_seen'], ['min', 'timestamp', 'first_seen'], ['max', 'timestamp', 'last_seen']], orderby='-first_seen', limit=1000, referrer='tagstore.get_group_tag_value_iter')
