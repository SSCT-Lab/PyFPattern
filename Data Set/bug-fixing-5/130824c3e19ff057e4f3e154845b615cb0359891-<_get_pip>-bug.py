    ...
    if (pip is None):
        if (env is None):
            ...
            for basename in candidate_pip_basenames:
                ...
                module.fail_json(msg=((('Unable to find pip in the virtualenv, %s, ' % env) + ('under any of these names: %s. ' % ', '.join(candidate_pip_basenames))) + 'Make sire pip is present in the virtualenv.'))
    return pip