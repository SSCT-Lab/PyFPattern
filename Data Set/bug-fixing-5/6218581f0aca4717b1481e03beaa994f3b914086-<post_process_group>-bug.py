...
def post_process_group(event, is_new, is_regression, is_sample, is_new_group_environment, **kwargs):
        ...
        if allowed_events:
            for (servicehook_id, events) in _get_service_hooks(project_id=event.project_id):
                if any(((e in allowed_events) for e in events)):
                    process_service_hook.delay(hook_id=servicehook_id, event=event)
    ...
    event_processed.send_robust(sender=post_process_group, project=event.project, group=event.group, event=event)