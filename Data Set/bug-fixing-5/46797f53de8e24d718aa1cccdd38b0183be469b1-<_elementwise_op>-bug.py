def _elementwise_op(helper):
    ...
    if (convert_dtype(x.dtype) in ['float16']):
        warnings.warn("The data type of 'x' in batch_norm only support float16 on GPU now.")
    if (convert_dtype(y.dtype) in ['float16']):
        warnings.warn("The data type of 'y' in batch_norm only support float16 on GPU now.")
    if (convert_dtype(x.dtype) not in ['float16', 'float32', 'float64', 'int32', 'int64']):
        raise TypeError(("The data type of 'x' in batch_norm must be float16 or float32 or float64 or int32 or int64, but received %s." % convert_dtype(x.dtype)))
    if (convert_dtype(y.dtype) not in ['float16', 'float32', 'float64', 'int32', 'int64']):
        raise TypeError(("The data type of 'y' in batch_norm must be float16 or float32 or float64 or int32 or int64, but received %s." % convert_dtype(y.dtype)))
