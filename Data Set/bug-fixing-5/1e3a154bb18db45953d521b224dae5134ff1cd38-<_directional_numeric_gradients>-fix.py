def _directional_numeric_gradients(self, directions, y0_data):
    ...
    xs_filtered = [x.array for (x, no_gx) in six.moves.zip(x_vars, no_gxs) if (not no_gx)]
    if (dtype is None):
        ...
        casted_data = [x.astype(dtype, copy=False) for x in (xs_filtered + params_data)]
    ...
    def g():
        ...
        return ys_data
    (gx,) = numerical_grad(g, (delta,), gys, eps=eps, detect_nondifferentiable=detect_nondifferentiable, center_outputs=y0_data, diff_atol=0, diff_rtol=self.rtol)
    return gx