def check_bgp_af_other_args(self, **kwargs):
    ...
    if max_load_ibgp_num:
        ...
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<maxLoadIbgpNum></maxLoadIbgpNum>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
    ...
    if (ibgp_ecmp_nexthop_changed != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<ibgpEcmpNexthopChanged></ibgpEcmpNexthopChanged>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if max_load_ebgp_num:
        ...
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<maxLoadEbgpNum></maxLoadEbgpNum>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (ebgp_ecmp_nexthop_changed != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<ebgpEcmpNexthopChanged></ebgpEcmpNexthopChanged>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if maximum_load_balance:
        ...
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<maximumLoadBalance></maximumLoadBalance>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (ecmp_nexthop_changed != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<ecmpNexthopChanged></ecmpNexthopChanged>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if default_local_pref:
        ...
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<defaultLocalPref></defaultLocalPref>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if default_med:
        ...
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<defaultMed></defaultMed>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (default_rt_import_enable != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<defaultRtImportEnable></defaultRtImportEnable>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if router_id:
        ...
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<routerId></routerId>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (vrf_rid_auto_sel != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<vrfRidAutoSel></vrfRidAutoSel>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (nexthop_third_party != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<nexthopThirdParty></nexthopThirdParty>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (summary_automatic != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<summaryAutomatic></summaryAutomatic>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (auto_frr_enable != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<autoFrrEnable></autoFrrEnable>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (load_balancing_as_path_ignore != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<loadBalancingAsPathIgnore></loadBalancingAsPathIgnore>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (rib_only_enable != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<ribOnlyEnable></ribOnlyEnable>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if rib_only_policy_name:
        ...
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<ribOnlyPolicyName></ribOnlyPolicyName>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (active_route_advertise != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<activeRouteAdvertise></activeRouteAdvertise>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (as_path_neglect != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<asPathNeglect></asPathNeglect>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (med_none_as_maximum != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<medNoneAsMaximum></medNoneAsMaximum>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (router_id_neglect != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<routerIdNeglect></routerIdNeglect>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (igp_metric_ignore != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<igpMetricIgnore></igpMetricIgnore>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (always_compare_med != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<alwaysCompareMed></alwaysCompareMed>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (determin_med != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<determinMed></determinMed>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if preference_external:
        ...
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<preferenceExternal></preferenceExternal>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if preference_internal:
        ...
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<preferenceInternal></preferenceInternal>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if preference_local:
        ...
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<preferenceLocal></preferenceLocal>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if prefrence_policy_name:
        ...
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<prefrencePolicyName></prefrencePolicyName>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (reflect_between_client != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<reflectBetweenClient></reflectBetweenClient>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if reflector_cluster_id:
        ...
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<reflectorClusterId></reflectorClusterId>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if reflector_cluster_ipv4:
        ...
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<reflectorClusterIpv4></reflectorClusterIpv4>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if rr_filter_number:
        ...
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<rrFilterNumber></rrFilterNumber>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (policy_vpn_target != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<policyVpnTarget></policyVpnTarget>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if next_hop_sel_depend_type:
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<nextHopSelDependType></nextHopSelDependType>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if nhp_relay_route_policy_name:
        ...
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<nhpRelayRoutePolicyName></nhpRelayRoutePolicyName>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (ebgp_if_sensitive != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<ebgpIfSensitive></ebgpIfSensitive>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (reflect_chg_path != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<reflectChgPath></reflectChgPath>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if add_path_sel_num:
        ...
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<addPathSelNum></addPathSelNum>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if route_sel_delay:
        ...
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<routeSelDelay></routeSelDelay>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (allow_invalid_as != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<allowInvalidAs></allowInvalidAs>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (policy_ext_comm_enable != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<policyExtCommEnable></policyExtCommEnable>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (supernet_uni_adv != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<supernetUniAdv></supernetUniAdv>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (supernet_label_adv != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<supernetLabelAdv></supernetLabelAdv>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if ingress_lsp_policy_name:
        ...
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<ingressLspPolicyName></ingressLspPolicyName>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (originator_prior != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<originatorPrior></originatorPrior>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (lowest_priority != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<lowestPriority></lowestPriority>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
    ...
    if (relay_delay_enable != 'no_use'):
        conf_str = (((CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name) + '<relayDelayEnable></relayDelayEnable>') + CE_GET_BGP_ADDRESS_FAMILY_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if ('<data/>' in recv_xml):
            need_cfg = True
        else:
