def assign_anchor(feat_shape, gt_boxes, im_info, feat_stride=16, scales=(8, 16, 32), ratios=(0.5, 1, 2), allowed_border=0):
    ...
    logger.debug(('anchor shapes: %s' % np.hstack(((base_anchors[:, 2::4] - base_anchors[:, 0::4]), (base_anchors[:, 3::4] - base_anchors[:, 1::4])))))
    ...
    inds_inside = np.where(((((all_anchors[:, 0] >= (- allowed_border)) & (all_anchors[:, 1] >= (- allowed_border))) & (all_anchors[:, 2] < (im_info[1] + allowed_border))) & (all_anchors[:, 3] < (im_info[0] + allowed_border))))[0]
