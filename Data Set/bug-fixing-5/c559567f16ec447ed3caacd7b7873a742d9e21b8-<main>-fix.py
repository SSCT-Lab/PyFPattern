def main():
    ...
    try:
        if (s3_url and ceph):
            ...
            connection = boto.connect_s3(host=ceph.hostname, port=ceph.port, is_secure=(ceph.scheme == 'https'), **aws_connect_params)
            flavour = 'ceph'
        elif is_fakes3(s3_url):
            fakes3 = urlparse.urlparse(s3_url)
            connection = S3Connection(is_secure=(fakes3.scheme == 'fakes3s'), host=fakes3.hostname, port=fakes3.port, **aws_connect_params)
