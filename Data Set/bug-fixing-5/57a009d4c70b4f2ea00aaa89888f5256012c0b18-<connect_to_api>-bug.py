def connect_to_api(module, disconnect_atexit=True):
    ...
    try:
        ...
        module.fail_json(msg=('User %s does not have required permission to log on to vCenter or ESXi API at %s:%s : %s' % (username, hostname, port, e.msg)))
    except (requests.ConnectionError, ssl.SSLError) as e:
        module.fail_json(msg=('Unable to connect to vCenter or ESXi API at %s on TCP/%s: %s' % (hostname, port, e)))
    except vmodl.fault.InvalidRequest as e:
        module.fail_json(msg=('Failed to get a response from server %s:%s as request is malformed: %s' % (hostname, port, e.msg)))
