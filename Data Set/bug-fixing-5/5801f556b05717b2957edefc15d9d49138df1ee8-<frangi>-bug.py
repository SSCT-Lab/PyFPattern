def frangi(image, sigmas=range(1, 10, 2), scale_range=None, scale_step=None, beta1=None, beta2=None, alpha=0.5, beta=0.5, gamma=15, black_ridges=True):
    ...
    if beta1:
        warn('Use keyword parameter `beta` instead of `beta1` which will be removed in version 0.17.')
        beta = beta1
    if beta2:
        warn('Use keyword parameter `gamma` instead of `beta2` which will be removed in version 0.17.')
        gamma = beta2
    if (scale_range and scale_step):
        warn('Use keyword parameter `sigmas` instead of `scale_range` and `scale_range` which will be removed in version 0.17.')
