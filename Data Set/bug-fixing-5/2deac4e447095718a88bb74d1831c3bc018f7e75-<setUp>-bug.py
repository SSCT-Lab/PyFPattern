def setUp(self):
    ...
    if (self.srctype == np.int8):
        ...
        if self.fuse_residual:
            ...
            output_tmp = np.round(((output1 * (self.scale_out / (self.scale_in * self.scale_weights[0]))) + (format_reorder(input_residual, self.input_residual_size).astype(np.int32) * (self.scale_out / self.scale_in_eltwise))))
            output_tmp2 = np.round((output1 * (self.scale_out / (self.scale_in * self.scale_weights[0]))))
            if self.fuse_relu:
                output = np.maximum(output_tmp, 0).astype(self.dsttype)
            else:
                output = output_tmp.astype(self.dsttype)
