def run_commit_log_consumer(bootstrap_servers, consumer_group, commit_log_topic, partition_state_manager, synchronize_commit_group, start_event, stop_request_event):
    ...
    while (not stop_request_event.is_set()):
        ...
        if (offset in LOGICAL_OFFSETS):
            logger.debug('Skipping invalid logical offset (%r) from %s/%s...', offset, topic, partition)
            continue
        elif (offset < 0):
            logger.warning('Received unexpected negative offset (%r) from %s/%s!', offset, topic, partition)
