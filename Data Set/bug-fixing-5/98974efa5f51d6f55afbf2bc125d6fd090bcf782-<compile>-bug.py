def compile(self, optimizer, loss, metrics=[], loss_weights=None, sample_weight_mode=None, **kwargs):
    ...
    if (loss_weights is None):
        ...
        if (len(loss_weights) != len(self.outputs)):
            raise Exception(((('When passing a list as loss_weights, it should have one entry per model outputs. The model has ' + str(len(self.outputs))) + ' outputs, but you passed loss_weights=') + str(loss)))
