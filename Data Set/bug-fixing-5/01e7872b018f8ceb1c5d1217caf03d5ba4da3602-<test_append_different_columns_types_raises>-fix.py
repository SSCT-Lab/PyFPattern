...
def test_append_different_columns_types_raises(self, index_can_append, index_cannot_append_with_other):
    ...
    msg = "unorderable types: (Interval|int)\\(\\) > (int|long|float|str)\\(\\)|Expected tuple, got (int|long|float|str)|Cannot compare type 'Timestamp' with type '(int|long)'|'>' not supported between instances of 'int' and 'str'"
