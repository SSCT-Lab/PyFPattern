    ...
    if update_all:
        ...
        for spec in items:
            if spec.startswith('@'):
                ...
                if (not is_installed(module, repoq, envra, conf_file, en_repos=en_repos, dis_repos=dis_repos, en_plugins=en_plugins, dis_plugins=dis_plugins, installroot=installroot)):
                ...
                continue
            elif (is_installed(module, repoq, spec, conf_file, en_repos=en_repos, dis_repos=dis_repos, en_plugins=en_plugins, dis_plugins=dis_plugins, installroot=installroot) or update_only):
                pkgs['update'].append(spec)
            else:
                pkgs['install'].append(spec)
            ...
            nothing_to_do = True
            for pkg in pkglist:
                if ((spec in pkgs['install']) and is_available(module, repoq, pkg, conf_file, en_repos=en_repos, dis_repos=dis_repos, en_plugins=en_plugins, dis_plugins=dis_plugins, installroot=installroot)):
                    nothing_to_do = False
                    break
                ...
                if ((spec in pkgs['update']) and (pkgname in updates)):
                    ...
                    break
            if ((not is_installed(module, repoq, spec, conf_file, en_repos=en_repos, dis_repos=dis_repos, en_plugins=en_plugins, dis_plugins=dis_plugins, installroot=installroot)) and update_only):
