...
def _ExtractImagePatchesGrad(op, grad):
    (batch_size, rows_in, cols_in, channels) = [dim.value for dim in op.inputs[0].get_shape()]
    ...
    grad_expanded = array_ops.transpose(array_ops.reshape(grad, (batch_size, rows_out, cols_out, ksize_r, ksize_c, channels)), (1, 2, 3, 4, 0, 5))
    ...
    for i in range(rows_out):
        for j in range(cols_out):
            ...
            idx.extend([(((r * cols_in) + c), ((((i * ((cols_out * ksize_r) * ksize_c)) + (j * (ksize_r * ksize_c))) + (ri * ksize_c)) + ci)) for (ri, r) in enumerate(range(r_low, r_high, rate_r)) for (ci, c) in enumerate(range(c_low, c_high, rate_c)) if ((0 <= r) and (r < rows_in) and (0 <= c) and (c < cols_in))])
    sp_shape = ((rows_in * cols_in), (((rows_out * cols_out) * ksize_r) * ksize_c))
    sp_mat = sparse_tensor.SparseTensor(array_ops.constant(idx, dtype=ops.dtypes.int64), array_ops.ones((len(idx),), dtype=ops.dtypes.float32), sp_shape)
