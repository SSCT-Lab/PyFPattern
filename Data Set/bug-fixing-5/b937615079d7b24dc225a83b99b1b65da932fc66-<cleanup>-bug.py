...
def cleanup(days, project, concurrency, silent, model, router, timed):
    ...
    for (model, dtfield, order_by) in DELETES:
        ...
        if is_filtered(model):
            ...
            task = deletions.get(model=model, query=query, order_by=order_by, skip_models=[models.Event, models.EventTag, models.GroupEmailThread, models.GroupRuleStatus, models.GroupTagValue], transaction_id=uuid4().hex)
