def converge_file_system(self, name, tags, targets):
    ...
    if (targets is not None):
        ...
        changed = filter((lambda sid: (not targets_equal(['SubnetId', 'IpAddress', 'NetworkInterfaceId'], current_targets[sid], targets[sid]))), intersection)
        ...
        if targets_to_create:
            ...
            result = True
        security_groups_to_update = filter((lambda sid: (('SecurityGroups' in targets[sid]) and (current_targets[sid]['SecurityGroups'] != targets[sid]['SecurityGroups']))), intersection)
        if security_groups_to_update:
            for sid in security_groups_to_update:
                self.connection.modify_mount_target_security_groups(MountTargetId=current_targets[sid]['MountTargetId'], SecurityGroups=targets[sid]['SecurityGroups'])
            result = True
    return result