def __get_template_with_version(self):
    ...
    if self.param('template'):
        ...
        templates = templates_service.list(search=('name=%s and cluster=%s' % (self.param('template'), cluster)))
        if (not templates):
        ...
        if (not templates):
            raise ValueError(("Template with name '%s' and version '%s' in cluster '%s' was not found'" % (self.param('template'), self.param('template_version'), cluster)))
