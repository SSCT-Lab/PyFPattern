def main():
    ...
    try:
        ...
        if (state in ('present', 'detached', 'attached')):
            ret = disks_module.create(entity=disk, result_state=(otypes.DiskStatus.OK if (lun is None) else None), fail_condition=(lambda d: ((d.status == otypes.DiskStatus.ILLEGAL) if (lun is None) else False)))
