def _real_extract(self, url):
    ...
    decoded_id = (get_element_by_id('streamurl', webpage) or get_element_by_id('streamuri', webpage) or get_element_by_id('streamurj', webpage) or self._search_regex(('>\\s*([\\w-]+~\\d{10,}~\\d+\\.\\d+\\.0\\.0~[\\w-]+)\\s*<', '>\\s*([\\w~-]+~\\d+\\.\\d+\\.\\d+\\.\\d+~[\\w~-]+)', '>\\s*([\\w-]+~\\d{10,}~(?:[a-f\\d]+:){2}:~[\\w-]+)\\s*<', '>\\s*([\\w~-]+~[a-f0-9:]+~[\\w~-]+)\\s*<', '>\\s*([\\w~-]+~[a-f0-9:]+~[\\w~-]+)'), webpage, 'stream URL'))
