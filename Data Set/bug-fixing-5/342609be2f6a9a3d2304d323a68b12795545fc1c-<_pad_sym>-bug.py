def _pad_sym(arr, pad_amt, method, axis=(- 1)):
    ...
    if (('odd' in method) and (pad_amt[0] > 0)):
        edge_slice1 = tuple(((slice(None) if (i != axis) else 0) for (i, x) in enumerate(arr.shape)))
    ...
    if ('odd' in method):
        edge_slice2 = tuple(((slice(None) if (i != axis) else (- 1)) for (i, x) in enumerate(arr.shape)))
