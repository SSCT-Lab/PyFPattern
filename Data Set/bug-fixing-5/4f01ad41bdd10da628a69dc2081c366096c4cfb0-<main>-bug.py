def main():
    ...
    try:
        ...
        if (state == 'present'):
            ret = hosts_module.create(deploy_hosted_engine=((module.params.get('hosted_engine') == 'deploy') if (module.params.get('hosted_engine') is not None) else None), result_state=(hoststate.UP if (host is None) else None), fail_condition=(failed_state if (host is None) else (lambda h: False)))
