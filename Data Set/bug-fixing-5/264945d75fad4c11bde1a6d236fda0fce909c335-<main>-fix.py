    ...
    try:
        ...
        if ((state == 'present') and ((nic is None) or host_networks_module.has_update(nics_service.service(nic.id)))):
            ...
            if ((labels and set(labels).intersection(attached_labels)) or bond or attachments):
                setup_params = dict(entity=host, action='setup_networks', check_connectivity=module.params['check'], removed_bonds=([otypes.HostNic(name=bond.get('name'))] if bond else None), removed_labels=([otypes.NetworkLabel(id=str(name)) for name in labels] if labels else None), removed_network_attachments=(attachments if attachments else None))
                if engine_supported(connection, '4.3'):
                    setup_params['commit_on_success'] = module.params['save']
                elif module.params['save']:
                    setup_params['post_action'] = host_networks_module._action_save_configuration
