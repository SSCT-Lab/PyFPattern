def do_layout(self, *largs):
    ...
    for c in reversed(self.children):
        if c.size_hint[outerattr]:
            c.size[outerattr] = max(1, (c.size_hint[outerattr] * (selfsize[outerattr] - padding_v)))
        ccount = len(lc)
        ...
        if (not lc):
            ...
            for (i, child) in enumerate(lc):
                ...
                if child.size_hint[innerattr]:
                    testsizes[i] = childsize = max(1, (child.size_hint[innerattr] * totalsize))
                else:
        ...
        lv = c.size[outerattr]
        if c.size_hint[innerattr]:
            sizes = [max(1, (c.size_hint[innerattr] * (selfsize[innerattr] - padding_u)))]
        else:
