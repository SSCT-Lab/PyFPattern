def _query(self, project, retention_window_start, group_queryset, tags, environment, sort_by, limit, cursor, count_hits, paginator_options, **parameters):
    ...
    if (environment is not None):
        ...
        group_tag_value_queryset = tagstore.get_group_tag_value_qs(project_id=project.id, group_id=set(group_queryset.values_list('id', flat=True)), environment_id=environment.id, key='environment', value=environment.name)
        if (retention_window_start is not None):
