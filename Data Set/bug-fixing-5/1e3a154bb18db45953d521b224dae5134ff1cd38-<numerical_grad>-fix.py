def numerical_grad(f, inputs, grad_outputs, eps=0.001, detect_nondifferentiable=False, diff_atol=0, diff_rtol=0.01, center_outputs=None):
    ...
    inputs = tuple(inputs)
    grad_outputs = tuple([(None if (g is None) else (numpy.float64(g) if numpy.isscalar(g) else g.astype(numpy.float64))) for g in grad_outputs])
    ...
    def eval_func(x, x_ind, delta, orig):
        ...
        assert all([(gy is None) for (y, gy) in zip(ys, grad_outputs) if (y is None)])
        assert all([((gy is None) or numpy.isscalar(gy) or (y.shape == gy.shape)) for (y, gy) in zip(ys, grad_outputs)])
        x[x_ind] = orig
        return ys
    ...
    def iterate_single_input(i_in, x, orig_x, x_ind):
        orig = orig_x[x_ind]
        if detect_nondifferentiable:
            yss = [eval_func(x, x_ind, ((- eps) * 1.0), orig), eval_func(x, x_ind, ((- eps) * 0.5), orig), ys0, eval_func(x, x_ind, ((+ eps) * 0.5), orig), eval_func(x, x_ind, ((+ eps) * 1.0), orig)]
        else:
            yss = [eval_func(x, x_ind, ((- eps) * 1), orig), eval_func(x, x_ind, ((+ eps) * 1), orig)]
        assert all([((y is None) or ((y.shape == yss[0][i].shape) and (y.dtype == yss[0][i].dtype))) for ys in yss for (i, y) in enumerate(ys)])
        ...
        if detect_nondifferentiable_:
            ...
            if (not any_nonfinite):
                ...
                for i_out in range(nout):
                    ...
                    det = utils.force_array(((diff_atol + (diff_rtol * (ymax - ymin))) < res))
                    det[(ymax == ymin)] = False
                    if det.any():
                        s = six.StringIO()
                        s.write('Tried to compute the numeric gradient on a non-differentiable point.\n\n')
        ...
        for (i_out, gy) in enumerate(grad_outputs):
            ...
            assert all([((yss[0][i_out] is None) == (yss[j][i_out] is None)) for j in range(len(yss))])
            ...
            if (len(yss) == 2):
                ...
                y1 = yss[1][i_out]
                if gpu_:
                    numerical_grad_kernel_1(y1, y0, xp.asarray(gy), eps, gx[x_ind])
                else:
                    dot = ((y1 - y0) * gy).sum()
