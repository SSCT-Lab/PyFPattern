    ...
    if (self._play_context.connection == 'local'):
        ...
        if (self._task.action in ['junos_netconf', 'junos_ping']):
            if (args_provider.get('transport') == 'netconf'):
                return {
                    'failed': True,
                    'msg': ("Transport type 'netconf' is not valid for '%s' module. Please see https://docs.ansible.com/ansible/latest/network/user_guide/platform_junos.html" % self._task.action),
                }
        elif ((provider['transport'] == 'netconf') and (self._task.action not in NETCONF_SUPPORTED_MODULES)):
            return {
                'failed': True,
