def ensure(module, client):
    ...
    if (state in ['present', 'disabled', 'enabled']):
        ...
        if (sudoopt is not None):
            changed = (client.modify_if_diff(name, ipa_sudorule.get('ipasudoopt', []), sudoopt, client.sudorule_add_option_ipasudoopt, client.sudorule_remove_option_ipasudoopt) or changed)
        if (user is not None):
