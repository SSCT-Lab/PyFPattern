def standardize_input_data(data, names, shapes=None, check_batch_dim=True, exception_prefix=''):
    ...
    if (type(data) is dict):
        ...
        for name in names:
            if (name not in data):
                raise Exception(((('No data provided for "' + name) + '". Need data for each key in: ') + str(data.keys())))
        ...
        if (len(data) != len(names)):
            if ((len(data) > 0) and hasattr(data[0], 'shape')):
                ...
                raise Exception(((((('Error when checking ' + exception_prefix) + ': you are passing a list as input to your model, but the model expects a list of ') + str(len(names))) + ' Numpy arrays instead. The list you passed was: ') + str(data)[:200]))
        arrays = data
    else:
        ...
        if (len(names) != 1):
            raise Exception(((('The model expects ' + str(len(names))) + ' input arrays, but only received one array. Found: array with shape ') + str(data.shape)))
        arrays = [data]
