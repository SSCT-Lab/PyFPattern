        ...
        if (self.vpn_target_state == 'present'):
            if (not self.is_vrf_rt_exist()):
                if (self.evpn is False):
                    self.updates_cmd.append(('vpn-target %s %s' % (self.vpn_target_value, self.vpn_target_type)))
                else:
                    self.updates_cmd.append(('vpn-target %s %s evpn' % (self.vpn_target_value, self.vpn_target_type)))
        elif (self.vpn_target_state == 'absent'):
            if self.is_vrf_rt_exist():
                if (self.evpn is False):
                    self.updates_cmd.append(('undo vpn-target %s %s' % (self.vpn_target_value, self.vpn_target_type)))
                else:
                    self.updates_cmd.append(('undo vpn-target %s %s evpn' % (self.vpn_target_value, self.vpn_target_type)))
    else:
        self.updates_cmd.append(('ip vpn-instance %s' % self.vrf))
        if (self.vrf_aftype == 'ipv4uni'):
