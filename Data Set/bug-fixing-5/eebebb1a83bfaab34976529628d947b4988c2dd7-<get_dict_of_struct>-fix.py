def get_dict_of_struct(connection, vm):
    ...
    return {
        ...
        'statistics': dict(((stat.name, stat.values[0].datum) for stat in stats if stat.values)),
        'devices': dict(((device.name, [ip.address for ip in device.ips]) for device in devices if device.ips)),
