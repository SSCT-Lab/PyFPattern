def check_bgp_instance_other_args(self, **kwargs):
    ...
    if router_id:
        ...
        conf_str = ((CE_GET_BGP_INSTANCE_HEADER + '<routerId></routerId>') + CE_GET_BGP_INSTANCE_TAIL)
    ...
    if (vrf_rid_auto_sel != 'no_use'):
        if (not vrf_name):
            module.fail_json(msg='Error: Please input vrf_name.')
        conf_str = ((CE_GET_BGP_INSTANCE_HEADER + '<vrfRidAutoSel></vrfRidAutoSel>') + CE_GET_BGP_INSTANCE_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if (state == 'present'):
            if ('<data/>' in recv_xml):
                need_cfg = True
    ...
    if keepalive_time:
        ...
        if ((int(keepalive_time) > 21845) or (int(keepalive_time) < 0)):
            ...
            conf_str = ((CE_GET_BGP_INSTANCE_HEADER + '<keepaliveTime></keepaliveTime>') + CE_GET_BGP_INSTANCE_TAIL)
            recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
            if (state == 'present'):
                if ('<data/>' in recv_xml):
                    need_cfg = True
    ...
    if hold_time:
        ...
        if ((int(hold_time) > 65535) or (int(hold_time) < 3)):
            ...
            conf_str = ((CE_GET_BGP_INSTANCE_HEADER + '<holdTime></holdTime>') + CE_GET_BGP_INSTANCE_TAIL)
            recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
            if (state == 'present'):
                if ('<data/>' in recv_xml):
                    need_cfg = True
    ...
    if min_hold_time:
        ...
        if ((int(min_hold_time) != 0) and ((int(min_hold_time) > 65535) or (int(min_hold_time) < 20))):
            ...
            conf_str = ((CE_GET_BGP_INSTANCE_HEADER + '<minHoldTime></minHoldTime>') + CE_GET_BGP_INSTANCE_TAIL)
            recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
            if (state == 'present'):
                if ('<data/>' in recv_xml):
                    need_cfg = True
    ...
    if conn_retry_time:
        ...
        if ((int(conn_retry_time) > 65535) or (int(conn_retry_time) < 1)):
            ...
            conf_str = ((CE_GET_BGP_INSTANCE_HEADER + '<connRetryTime></connRetryTime>') + CE_GET_BGP_INSTANCE_TAIL)
            recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
            if (state == 'present'):
                if ('<data/>' in recv_xml):
                    need_cfg = True
    ...
    if (ebgp_if_sensitive != 'no_use'):
        if (not vrf_name):
            module.fail_json(msg='Error: Please input vrf_name.')
        conf_str = ((CE_GET_BGP_INSTANCE_HEADER + '<ebgpIfSensitive></ebgpIfSensitive>') + CE_GET_BGP_INSTANCE_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if (state == 'present'):
            if ('<data/>' in recv_xml):
                need_cfg = True
    ...
    if default_af_type:
        if (not vrf_name):
            module.fail_json(msg='Error: Please input vrf_name.')
        conf_str = ((CE_GET_BGP_INSTANCE_HEADER + '<defaultAfType></defaultAfType>') + CE_GET_BGP_INSTANCE_TAIL)
        recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
        if (state == 'present'):
            if ('<data/>' in recv_xml):
                need_cfg = True
