def handle_album(self, album, write):
    ...
    if (any([self.should_use_r128(item) for item in album.items()]) and (not all([self.should_use_r128(item) for item in album.items()]))):
        raise ReplayGainError('Mix of ReplayGain and EBU R128 detected for some tracks in album {0}'.format(album))
