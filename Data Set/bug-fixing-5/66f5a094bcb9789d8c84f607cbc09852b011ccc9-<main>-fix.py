def main():
    ...
    argument_spec.update(dict(state=dict(required=True, choices=['present', 'absent']), name=dict(), cert=dict(), key=dict(no_log=True), cert_chain=dict(), new_name=dict(), path=dict(default='/'), new_path=dict(), dup_ok=dict(type='bool')))
    ...
    if ((state == 'present') and (not new_name) and (not new_path)):
        (cert, key, cert_chain) = load_data(cert=module.params.get('cert'), key=module.params.get('key'), cert_chain=module.params.get('cert_chain'))
    else:
