    ...
    while True:
        ...
        if (state.run_state == self.ITERATING_SETUP):
            if (not state.pending_setup):
                ...
                if (((gathering == 'implicit') and implied) or ((gathering == 'explicit') and boolean(self._play.gather_facts)) or ((gathering == 'smart') and implied and (not self._variable_manager._fact_cache.get(host.name, {
                    
                }).get('module_setup', False)))):
