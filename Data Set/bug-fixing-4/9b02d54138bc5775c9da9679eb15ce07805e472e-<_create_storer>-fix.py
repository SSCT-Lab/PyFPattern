def _create_storer(self, group, format=None, value=None, encoding: str='UTF-8', errors: str='strict') -> Union[('GenericFixed', 'Table')]:
    ...
    if ('table' not in pt):
        try:
            return globals()[_STORER_MAP[pt]](self, group, encoding=encoding, errors=errors)
        except KeyError:
    ...
    if (tt is None):
        if (value is not None):
            ...
            try:
                ...
                pass
    try:
        return globals()[_TABLE_MAP[tt]](self, group, encoding=encoding, errors=errors)
    except KeyError:
