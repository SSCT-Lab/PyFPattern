def __get_tag_keys_for_projects(self, projects, group_id, environments, start, end, limit=1000, keys=None, **kwargs):
    filters = {
        'project_id': projects,
    }
    if environments:
    ...
    result = snuba.query(start, end, ['tags_key'], conditions, filters, aggregations, limit=limit, orderby='-values_seen', referrer='tagstore.__get_tag_keys', **kwargs)
    if (group_id is None):
        ctor = TagKey
    else:
