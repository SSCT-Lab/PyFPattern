def load_dict(self, param_dict, ctx=None, allow_missing=False, ignore_extra=False, restore_prefix='', filename=None, cast_dtype=False, dtype_source='current'):
    ...
    if (not allow_missing):
        for name in self.keys():
            assert (name in arg_dict), ("Parameter '%s' is missing in %s, which contains parameters: %s. Please make sure source and target networks have the same prefix." % (name[lprefix:], error_str, _brief_print_list(arg_dict.keys())))
    for name in arg_dict:
        if (name not in self._params):
            assert ignore_extra, ("Parameter '%s' loaded from %s is not present in ParameterDict, choices are: %s. Set ignore_extra to True to ignore. Please make sure source and target networks have the same prefix." % (name[lprefix:], error_str, _brief_print_list(self._params.keys())))
            continue
