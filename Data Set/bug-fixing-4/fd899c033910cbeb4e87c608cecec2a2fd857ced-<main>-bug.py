    ...
    try:
        ...
        if ((state == 'present') or (state == 'running') or (state == 'next_run')):
            ...
            if module.params['force']:
                ret = vms_module.action(action='stop', post_action=vms_module._attach_cd, action_condition=(lambda vm: (vm.status != otypes.VmStatus.DOWN)), wait_condition=(lambda vm: (vm.status == otypes.VmStatus.DOWN)))
            else:
                ret = vms_module.action(action='shutdown', pre_action=vms_module._pre_shutdown_action, post_action=vms_module._attach_cd, action_condition=(lambda vm: (vm.status != otypes.VmStatus.DOWN)), wait_condition=(lambda vm: (vm.status == otypes.VmStatus.DOWN)))
