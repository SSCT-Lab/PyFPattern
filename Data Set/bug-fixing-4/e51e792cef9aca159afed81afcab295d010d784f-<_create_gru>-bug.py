...
def _create_gru(cls, init_model, pred_model, n, opset_version):
    ...
    def make_gru(direction_offset):
        ...
        if (direction_offset == 1):
            input = pred_mh.net.ReversePackedSegs([input_blob, sequence_lens], (name + '/input-reversed'))
        else:
            input = input_blob
        ...
        if (direction_offset == 1):
            hidden_t_all = pred_mh.net.ReversePackedSegs([hidden_t_all, sequence_lens], (name + '/output-reversed'))
