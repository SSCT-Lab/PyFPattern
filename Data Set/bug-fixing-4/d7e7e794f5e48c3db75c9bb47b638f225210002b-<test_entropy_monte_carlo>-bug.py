def test_entropy_monte_carlo(self):
    ...
    for (Dist, params) in EXAMPLES:
        for (i, param) in enumerate(params):
            ...
            self.assertEqual(actual, expected, prec=0.2, message='\n'.join(['{} example {}/{}, incorrect .entropy().'.format(Dist.__name__, i, len(params)), 'Expected (monte carlo) {}'.format(expected), 'Actual (analytic) {}'.format(actual), 'max error = {}'.format(torch.abs((actual - expected)).max())]))