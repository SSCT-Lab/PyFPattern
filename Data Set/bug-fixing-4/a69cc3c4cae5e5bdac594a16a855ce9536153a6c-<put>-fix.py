def put(self, request, user, notification_type):
    ...
    if (notification_type == 'reports'):
        ...
        for (org_id, enabled) in request.data.items():
            ...
            if (org_id not in org_ids):
                return Response({
                    'detail': ('User does not belong to at least one of the                             requested orgs (org_id: %s).' % org_id),
    ...
    if (not ids_to_update.issubset(parent_ids)):
        ...
        return Response({
            'detail': ('At least one of the requested projects is not                     available to this user, because the user does not belong                     to the necessary teams. (ids of unavailable projects: %s)' % bad_ids),
        }, status=status.HTTP_403_FORBIDDEN)
