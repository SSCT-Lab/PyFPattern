def _container_create_clone(self):
    ...
    if (container_state != 'stopped'):
        ...
        self.container.stop()
    build_command = [self.module.get_bin_path('lxc-clone', True)]
    build_command = self._add_variables(variables_dict=self._get_vars(variables=LXC_COMMAND_MAP['clone']['variables']), build_command=build_command)
