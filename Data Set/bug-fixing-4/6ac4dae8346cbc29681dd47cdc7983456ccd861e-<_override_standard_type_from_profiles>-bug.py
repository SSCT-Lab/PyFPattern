def _override_standard_type_from_profiles(self):
    ...
    if (self.want.type == 'standard'):
        if self.want.has_fastl4_profiles:
            ...
            self.module.deprecate(msg="Specifying 'performance-l4' profiles on a 'standard' type is deprecated and will be removed.", version='2.6')
        if self.want.has_fasthttp_profiles:
            self.want.update({
                'type': 'performance-http',
            })
            self.module.deprecate(msg="Specifying 'performance-http' profiles on a 'standard' type is deprecated and will be removed.", version='2.6')
        if self.want.has_message_routing_profiles:
            self.want.update({
                'type': 'message-routing',
            })
            self.module.deprecate(msg="Specifying 'message-routing' profiles on a 'standard' type is deprecated and will be removed.", version='2.6')