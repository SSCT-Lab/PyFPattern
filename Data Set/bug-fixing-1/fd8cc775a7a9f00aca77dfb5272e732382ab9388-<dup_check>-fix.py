            ...
            if (cert is not None):
                try:
                    ...
                    if (slug_orig_cert_bodies == slug_cert):
                        ...
                        module.fail_json(changed=False, msg=('A cert with the name %s already exists and has a different certificate body associated with it. Certificates cannot have the same name' % orig_cert_names[c_index]))
            else:
                update = True
                break
