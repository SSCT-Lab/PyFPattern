...
def create_or_update_role(connection, module):
    ...
    if ((not role.get('MadeInCheckMode')) and params.get('Description') and (role['Description'] != params['Description'])):
        try:
            ...
            module.fail_json(msg='Unable to update description for role {0}: {1}'.format(params['RoleName'], to_native(e)), exception=traceback.format_exc())
    if ((not role.get('MadeInCheckMode')) and ((role.get('PermissionsBoundary') or {
        
    }).get('PermissionsBoundaryArn') or (params.get('PermissionsBoundary') is not None))):
        if (module.params.get('boundary') is None):
            pass
        elif (module.params.get('boundary') == ''):
            if (role.get('PermissionsBoundary') or {
        ...
        elif ((role.get('PermissionsBoundary') or {
            
        }).get('PermissionsBoundaryArn') != params['PermissionsBoundary']):
            try:
                if (not module.check_mode):
