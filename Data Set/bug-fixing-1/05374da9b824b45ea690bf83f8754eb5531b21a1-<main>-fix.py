...
def main():
    ...
    aci.get_existing()
    if (state == 'present'):
        aci.payload(aci_class='infraHPortS', class_config=dict(descr=description, name=access_port_selector), child_configs=[dict(infraPortBlk=dict(attributes=dict(descr=leaf_port_blk_description, name=leaf_port_blk, fromPort=from_port, toPort=to_port, fromCard=from_card, toCard=to_card))), dict(infraRsAccBaseGrp=dict(attributes=dict(tDn=INTERFACE_TYPE_MAPPING[interface_type])))])
