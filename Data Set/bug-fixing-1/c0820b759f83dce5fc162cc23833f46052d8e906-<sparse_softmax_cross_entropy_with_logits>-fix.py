...
def sparse_softmax_cross_entropy_with_logits(logits, labels, name=None):
    ...
    with ops.op_scope([labels, logits], name, 'SparseSoftmaxCrossEntropyWithLogits'):
        ...
        if ((logits.get_shape().ndims is not None) and ((labels_static_shape.ndims is not None) and (labels_static_shape.ndims != (logits.get_shape().ndims - 1)))):
            raise ValueError(('Rank mismatch: Labels rank (received %s) should equal logits rank (received %s) - 1.' % (labels_static_shape.ndims, logits.get_shape().ndims)))
