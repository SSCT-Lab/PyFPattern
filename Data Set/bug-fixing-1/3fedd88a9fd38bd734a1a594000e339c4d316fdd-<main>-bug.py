...
def main():
    ...
    if (module.params['nat_destination'] and (StrictVersion(shade.__version__) < StrictVersion('1.8.0'))):
        module.fail_json(msg='To utilize nat_destination, the installed version ofthe shade library MUST be >= 1.8.0')
        ...
        if (state == 'present'):
            ...
            if f_ip:
                if network:
                    ...
                    network_id = None
                if all([((fixed_address and (f_ip.fixed_ip_address == fixed_address)) or (nat_destination and (f_ip.internal_network == fixed_address))), network, (f_ip.network != network_id)]):
