...
def main():
    module = AnsibleModule(argument_spec=dict(path=dict(type='list', required=True), format=dict(choices=['gz', 'bz2', 'zip', 'tar'], default='gz', required=False), dest=dict(required=False, type='path'), exclude_path=dict(type='list', required=False), remove=dict(required=False, default=False, type='bool')), add_file_common_args=True, supports_check_mode=True)
    ...
    if ((len(missing) == len(expanded_paths)) and dest and os.path.exists(dest)):
        if re.search('(\\.tar|\\.tar\\.gz|\\.tgz|.tbz2|\\.tar\\.bz2|\\.zip)$', os.path.basename(dest), re.IGNORECASE):
            state = 'archive'
        else:
            state = 'compress'
    elif archive:
