    ...
    if isinstance(data, dict):
        ...
        if (len(data) != len(names)):
            if (data and hasattr(data[0], 'shape')):
                raise ValueError((((((((('Error when checking model ' + exception_prefix) + ': the list of Numpy arrays that you are passing to your model is not the size the model expected. Expected to see ') + str(len(names))) + ' arrays but instead got the following list of ') + str(len(data))) + ' arrays: ') + str(data)[:200]) + '...'))
            elif (len(names) == 1):
