...
def main():
    ...
    if (exact_count is not None):
        (changed, instances_list, tagged_instances_list) = create_exact_count_of_vms(module, one_client, template_id, exact_count, attributes, count_attributes, labels, count_labels, disk_size, networks, hard, wait, wait_timeout, put_vm_on_hold, persistent)
        vms = tagged_instances_list
    elif ((template_id is not None) and (state == 'present')):
        (changed, instances_list, tagged_instances_list) = create_count_of_vms(module, one_client, template_id, count, attributes, labels, disk_size, networks, wait, wait_timeout, put_vm_on_hold, persistent)
        vms = instances_list
    else:
