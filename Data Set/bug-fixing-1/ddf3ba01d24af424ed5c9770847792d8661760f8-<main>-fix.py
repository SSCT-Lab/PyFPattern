...
def main():
    ...
    aci.get_existing()
    if (state == 'present'):
        aci.payload(aci_class='fabricNodeIdentP', class_config=dict(descr=description, name=switch, nodeId=node_id, podId=pod_id, dn='uni/controller/nodeidentpol/nodep-{0}'.format(serial), role=role, serial=serial))
