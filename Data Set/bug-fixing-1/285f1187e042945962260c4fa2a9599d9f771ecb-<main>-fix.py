...
def main():
    ...
    try:
        ...
        if (state == 'present'):
            ...
            ret = hosts_module.action(action='upgrade', action_condition=(lambda h: h.update_available), wait_condition=(lambda h: (h.status == result_state)), post_action=(lambda h: time.sleep(module.params['poll_interval'])), fail_condition=hosts_module.failed_state_after_reinstall)
