...
def get_client_config(request=None):
    ...
    context = {
        ...
        'sentryConfig': {
            ...
            'whitelistUrls': (settings.SENTRY_FRONTEND_WHITELIST_URLS if settings.SENTRY_FRONTEND_WHITELIST_URLS else list(('' if (settings.ALLOWED_HOSTS == ['*']) else settings.ALLOWED_HOSTS))),
        },
    }
    if (user and user.is_authenticated()):
        context.update({
