...
def pool3d(x, pool_size, strides=(1, 1, 1), padding='valid', data_format=None, pool_mode='max'):
    ...
    if (pool_mode == 'max'):
        pool_out = pool.pool_3d(x, ws=pool_size, stride=strides, ignore_border=True, pad=padding, mode='max')
    elif (pool_mode == 'avg'):
        pool_out = pool.pool_3d(x, ws=pool_size, stride=strides, ignore_border=True, pad=padding, mode='average_exc_pad')
    else:
