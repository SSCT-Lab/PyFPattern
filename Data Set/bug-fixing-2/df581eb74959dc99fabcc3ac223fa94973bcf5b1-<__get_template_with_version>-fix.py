...
def __get_template_with_version(self):
    ...
    if self.param('template'):
        templates = templates_service.list(search=('name=%s and cluster=%s' % (self.param('template'), self.param('cluster'))))
        if self.param('template_version'):
            templates = [t for t in templates if (t.version.version_number == self.param('template_version'))]
        if (not templates):
            raise ValueError(("Template with name '%s' and version '%s' in cluster '%s' was not found'" % (self.param('template'), self.param('template_version'), self.param('cluster'))))
