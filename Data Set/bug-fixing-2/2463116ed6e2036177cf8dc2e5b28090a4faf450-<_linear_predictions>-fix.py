...
def _linear_predictions(self, examples):
    ...
    with name_scope('sdca/prediction'):
        ...
        for (sfc, sv) in zip(examples['sparse_features'], sparse_variables):
            result_sparse += math_ops.segment_sum(math_ops.multiply(array_ops.gather(sv, sfc.feature_indices), sfc.feature_values), sfc.example_indices)
        dense_features = self._convert_n_to_tensor(examples['dense_features'])
        dense_variables = self._convert_n_to_tensor(self._variables['dense_features_weights'])
        result_dense = 0.0
        for i in range(len(dense_variables)):
            result_dense += math_ops.matmul(dense_features[i], array_ops.expand_dims(dense_variables[i], (- 1)))
