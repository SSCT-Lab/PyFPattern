...
def sanitize_network_params(self):
    ...
    for network in self.params['networks']:
        ...
        if (('name' in network) and (find_obj(self.content, [vim.Network], network['name']) is None)):
            ...
            for dvp in dvps:
                if (hasattr(dvp.config.defaultPortConfig, 'vlan') and isinstance(dvp.config.defaultPortConfig.vlan.vlanId, int) and (str(dvp.config.defaultPortConfig.vlan.vlanId) == network['vlan'])):
