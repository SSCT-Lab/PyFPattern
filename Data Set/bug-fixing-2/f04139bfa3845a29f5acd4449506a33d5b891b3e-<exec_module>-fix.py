        ...
        if (self.state == 'present'):
            ...
            if self.network_interface_names:
                ...
                if (set(current_nics) != set(network_interfaces)):
                    ...
                    updated_nics = [dict(id=id, primary=(i is 0)) for (i, id) in enumerate(network_interfaces)]
    ...
    if changed:
        if (self.state == 'present'):
            ...
            if (not vm):
                ...
                nics = [self.compute_models.NetworkInterfaceReference(id=id, primary=(i is 0)) for (i, id) in enumerate(network_interfaces)]
                ...
                nics = [self.compute_models.NetworkInterfaceReference(id=interface['id'], primary=(i is 0)) for (i, interface) in enumerate(vm_dict['properties']['networkProfile']['networkInterfaces'])]
                ...
                try:
                    ...
                    pass
                vm_resource = self.compute_models.VirtualMachine(vm_dict['location'], os_profile=self.compute_models.OSProfile(admin_username=vm_dict['properties'].get('osProfile', {
                    
                }).get('adminUsername'), computer_name=vm_dict['properties'].get('osProfile', {
                    
                }).get('computerName')), hardware_profile=self.compute_models.HardwareProfile(vm_size=vm_dict['properties']['hardwareProfile'].get('vmSize')), storage_profile=self.compute_models.StorageProfile(os_disk=self.compute_models.OSDisk(name=vm_dict['properties']['storageProfile']['osDisk'].get('name'), vhd=vhd, managed_disk=managed_disk, create_option=vm_dict['properties']['storageProfile']['osDisk'].get('createOption'), os_type=vm_dict['properties']['storageProfile']['osDisk'].get('osType'), caching=vm_dict['properties']['storageProfile']['osDisk'].get('caching'), disk_size_gb=vm_dict['properties']['storageProfile']['osDisk'].get('diskSizeGB')), image_reference=(self.compute_models.ImageReference(id=vm_dict['properties']['storageProfile']['imageReference']['id']) if ('id' in vm_dict['properties']['storageProfile']['imageReference'].keys()) else self.compute_models.ImageReference(publisher=vm_dict['properties']['storageProfile']['imageReference'].get('publisher'), offer=vm_dict['properties']['storageProfile']['imageReference'].get('offer'), sku=vm_dict['properties']['storageProfile']['imageReference'].get('sku'), version=vm_dict['properties']['storageProfile']['imageReference'].get('version')))), availability_set=availability_set_resource, network_profile=self.compute_models.NetworkProfile(network_interfaces=nics))
                if vm_dict.get('tags'):
