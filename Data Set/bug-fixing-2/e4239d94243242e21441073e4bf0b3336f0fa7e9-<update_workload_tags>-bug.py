...
def update_workload_tags(self, check_mode=False):
    ...
    if self.workload_name:
        ...
        tag_index = (max([int(pair['value'].replace('ansible_workload_', '')) for tag in workload_tags for pair in tag['workloadAttributes'] if ((pair['key'] == 'profileId') and ('ansible_workload_' in pair['value']) and str(pair['value']).replace('ansible_workload_', '').isdigit())]) + 1)
