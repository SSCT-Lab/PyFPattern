...
def save_paddle_inference_model(self, executor, scope, program, feeded_vars, target_vars, output_path, day, pass_id, hadoop_fs_name, hadoop_fs_ugi, hadoop_home='$HADOOP_HOME', save_combine=True):
    ...
    if (fleet.worker_index() == 0):
        with fluid.scope_guard(scope):
            if save_combine:
                fluid.io.save_inference_model(dirname=model_name, feeded_var_names=feeded_var_names, target_vars=target_vars, executor=executor, main_program=program, params_filename='params')
            else:
                fluid.io.save_inference_model(dirname=model_name, feeded_var_names=feeded_var_names, target_vars=target_vars, executor=executor, main_program=program)
        configs = {
