    ...
    def merge_dicts(dict1, dict2):
        ...
        for (k, v) in dict2.items():
            ...
            if ((k not in merged) or (isinstance(v, compat_str) and v and isinstance(merged[k], compat_str) and (not merged[k]))):
                merged[k] = v
        return merged
    sharevideosse_urls = [m.group('url') for m in re.finditer('<iframe[^>]+?src\\s*=\\s*(["\\\'])(?P<url>https?://embed\\.share-videos\\.se/auto/embed/\\d+.+?)\\1', webpage)]
    if sharevideosse_urls:
        return self.playlist_from_matches(sharevideosse_urls, video_id, video_title)
