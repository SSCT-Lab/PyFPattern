...
def test_root_non_number(self):
    with pytest.raises(AnsibleFilterError, match="root\\(\\) can only be used on numbers: (invalid literal for float\\(\\): a|could not convert string to float: a|could not convert string to float: 'a')"):
