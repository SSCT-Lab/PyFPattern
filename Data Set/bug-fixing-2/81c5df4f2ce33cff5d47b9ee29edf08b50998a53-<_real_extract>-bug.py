...
def _real_extract(self, url):
    ...
    for (num, code) in enumerate(codes, 1):
        jwplayer_data = self._parse_json(self._search_regex('setup\\(([^)]+)\\)', code, 'jwplayer data', default=(NO_DEFAULT if (num == len(codes)) else '{}')), video_id, transform_source=js_to_json)
        if jwplayer_data:
            break
