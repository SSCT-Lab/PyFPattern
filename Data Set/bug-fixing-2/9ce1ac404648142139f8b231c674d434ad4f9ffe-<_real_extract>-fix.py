...
def _real_extract(self, url):
    ...
    try:
        ...
        if (doc.tag == 'rss'):
            ...
            info_dict['formats'] = self._parse_mpd_formats(doc, video_id, mpd_base_url=compat_str(full_response.geturl()).rpartition('/')[0], mpd_url=url)
            self._sort_formats(info_dict['formats'])
            return info_dict
    ...
    matches = re.findall('<iframe[^>]+?src="((?:https?:)?//(?:www\\.)?rtl\\.nl/system/videoplayer/[^"]+(?:video_)?embed[^"]+)"', webpage)
    if matches:
    ...
    mobj = re.search(('<iframe[^>]+src="(?P<url>%s)"' % UDNEmbedIE._PROTOCOL_RELATIVE_VALID_URL), webpage)
    if (mobj is not None):
