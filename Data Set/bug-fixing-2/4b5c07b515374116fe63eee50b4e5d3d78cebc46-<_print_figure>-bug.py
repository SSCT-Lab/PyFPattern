...
def _print_figure(self, outfile, format, dpi=72, facecolor='w', edgecolor='w', orientation='portrait', isLandscape=False, papertype=None, metadata=None, *, dryrun=False, bbox_inches_restore=None, **kwargs):
            ...
            for (font_filename, chars) in ps_renderer.used_characters.values():
                if len(chars):
                    ...
                    if is_opentype_cff_font(font_filename):
                        raise RuntimeError('OpenType CFF fonts can not be saved using the internal Postscript backend at this time; consider using the Cairo backend')
                    else:
                        fh.flush()
                        convert_ttf_to_ps(os.fsencode(font_filename), fh, fonttype, glyph_ids)
        print('end', file=fh)
