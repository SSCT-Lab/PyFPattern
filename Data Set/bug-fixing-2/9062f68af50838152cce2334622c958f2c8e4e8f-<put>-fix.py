...
def put(self, request, group, integration_id):
    ...
    (external_issue, created) = ExternalIssue.objects.get_or_create(organization_id=organization_id, integration_id=integration.id, key=external_issue_id, defaults=defaults)
    if (not created):
