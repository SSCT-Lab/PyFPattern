        ...
        for i in range(out1[0].shape[0]):
            for j in range(out1[0].shape[1]):
                ...
                for k in range(out2.shape[3]):
                    ...
                    p = (p + 2)
    assert_almost_equal(a, out1[0], rtol=0.001, atol=1e-05)
    if (len(shape) == 2):
        ...
        assert_almost_equal(a.real, (exe.grad_arrays[0].asnumpy() / shape[1]), rtol=0.001, atol=1e-05)
    if (len(shape) == 4):
        ...
        for exe in exe_list:
            exe.backward([out_grad])
        a = np.fft.ifft(out_grad_complex, n=None, axis=(- 1), norm=None)
        assert_almost_equal(a.real, (exe.grad_arrays[0].asnumpy() / shape[3]), rtol=0.001, atol=1e-05)