def main():
    ...
    if ((state == 'absent') and cert_present):
        ...
        if path:
            import_cert_path(module, executable, path, keystore_path, keystore_pass, cert_alias, keystore_type, trust_cacert)
        if url:
            import_cert_url(module, executable, url, port, keystore_path, keystore_pass, cert_alias, keystore_type, trust_cacert)
