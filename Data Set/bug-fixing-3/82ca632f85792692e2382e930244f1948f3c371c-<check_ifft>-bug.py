def check_ifft(shape):
    ...
    if (len(shape) == 2):
        ...
        assert_almost_equal(a.real, (out1[0] / shape_old[1]), rtol=0.001, atol=1e-12)
    if (len(shape) == 4):
        init_complex = np.zeros(shape_old, dtype=np.complex64)
        for i in range(0, shape_old[3]):
            ...
            init_complex.imag[:, :, :, i] = init[0][:, :, :, ((2 * i) + 1)]
        a = np.fft.ifft(init_complex, n=None, axis=(- 1), norm=None)
        assert_almost_equal(a.real, (out1[0] / shape_old[3]), rtol=0.001, atol=1e-12)
    if (len(shape) == 2):
        ...
        assert_almost_equal(a.real, temp, rtol=0.001, atol=1e-12)
    if (len(shape) == 4):
        out_grad = mx.nd.empty(shape_old)
        out_grad[:] = np.random.normal((- 3), 3, shape_old)
        for exe in exe_list:
