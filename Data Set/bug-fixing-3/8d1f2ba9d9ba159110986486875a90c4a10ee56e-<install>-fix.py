def install(module, items, repoq, yum_basecmd, conf_file, en_repos, dis_repos, en_plugins, dis_plugins, installroot='/', allow_downgrade=False):
    ...
    for spec in items:
        ...
        if spec.endswith('.rpm'):
            ...
            installed_pkgs = is_installed(module, repoq, envra, conf_file, en_repos=en_repos, dis_repos=dis_repos, en_plugins=en_plugins, dis_plugins=dis_plugins, installroot=installroot)
            if installed_pkgs:
                res['results'].append(('%s providing %s is already installed' % (installed_pkgs[0], package)))
                continue
            (name, ver, rel, epoch, arch) = splitFilename(envra)
            installed_pkgs = is_installed(module, repoq, name, conf_file, en_repos=en_repos, dis_repos=dis_repos, en_plugins=en_plugins, dis_plugins=dis_plugins, installroot=installroot)
            ...
            if (not set(['*', '?']).intersection(set(spec))):
                installed_pkgs = is_installed(module, repoq, spec, conf_file, en_repos=en_repos, dis_repos=dis_repos, en_plugins=en_plugins, dis_plugins=dis_plugins, is_pkg=True, installroot=installroot)
                if installed_pkgs:
                    res['results'].append(('%s providing %s is already installed' % (installed_pkgs[0], spec)))
                    continue
            pkglist = what_provides(module, repoq, yum_basecmd, spec, conf_file, en_repos=en_repos, dis_repos=dis_repos, en_plugins=en_plugins, dis_plugins=dis_plugins, installroot=installroot)
            if (not pkglist):
            ...
            found = False
            for this in pkglist:
                if is_installed(module, repoq, this, conf_file, en_repos=en_repos, dis_repos=dis_repos, en_plugins=en_plugins, dis_plugins=dis_plugins, is_pkg=True, installroot=installroot):
                    found = True
                    res['results'].append(('%s providing %s is already installed' % (this, spec)))
                    break
            if (not found):
                if is_installed(module, repoq, spec, conf_file, en_repos=en_repos, dis_repos=dis_repos, en_plugins=en_plugins, dis_plugins=dis_plugins, installroot=installroot):
                    found = True
            ...
            if allow_downgrade:
                for package in pkglist:
                    ...
                    inst_pkgs = is_installed(module, repoq, name, conf_file, en_repos=en_repos, dis_repos=dis_repos, en_plugins=en_plugins, dis_plugins=dis_plugins, is_pkg=True)
                    if inst_pkgs:
