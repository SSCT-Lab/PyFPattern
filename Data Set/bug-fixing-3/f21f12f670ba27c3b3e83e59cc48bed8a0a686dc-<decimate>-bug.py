def decimate(x, q, n=None, ftype='iir', axis=(- 1), zero_phase=None):
    ...
    if (zero_phase is None):
        warnings.warn(" Note: Decimate's zero_phase keyword argument will default to True in a future release. Until then, decimate defaults to one-way filtering for backwards compatibility. Ideally, always set this argument explicitly.", FutureWarning)
        zero_phase = False
