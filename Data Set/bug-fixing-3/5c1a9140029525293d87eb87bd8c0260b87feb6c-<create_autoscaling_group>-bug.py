    ...
    if (not as_groups.get('AutoScalingGroups')):
        ...
        if (load_balancers and (not as_group['LoadBalancerNames'])):
            ...
            try:
                ...
                module.fail_json(msg='Failed to update Autoscaling Group.', exception=traceback.format_exc(), **camel_dict_to_snake_dict(e.message))
        ...
        if notification_topic:
            try:
                ...
                module.fail_json(msg='Failed to update Autoscaling Group notifications.', exception=traceback.format_exc(), **camel_dict_to_snake_dict(e.message))
        if wait_for_instances:
            wait_for_new_inst(module, connection, group_name, wait_timeout, desired_capacity, 'viable_instances')
            if load_balancers:
        ...
        try:
            ...
            module.fail_json(msg='Failed to read existing Autoscaling Groups.', exception=traceback.format_exc(), **camel_dict_to_snake_dict(e.message))
        return (changed, asg_properties)