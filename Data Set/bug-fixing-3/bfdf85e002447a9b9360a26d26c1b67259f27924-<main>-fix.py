    ...
    if (iam_type == 'user'):
        ...
        if ((state == 'present') and (not user_exists) and (not new_name)):
            ...
            if ((name not in orig_user_list) and (new_name in orig_user_list)):
                been_updated = True
            (name_change, key_list, user_changed, new_key) = update_user(module, iam, name, new_name, new_path, key_state, key_count, key_ids, password, been_updated)
            if new_key:
                user_meta = {
                    'access_keys': list(new_key),
                }
                user_meta['access_keys'].extend([{
                    'access_key_id': key,
                    'status': value,
                } for (key, value) in key_list.items() if (key not in [it['access_key_id'] for it in new_key])])
            else:
                user_meta = {
                    'access_keys': [{
                        'access_key_id': key,
            ...
            if (new_name and new_path):
                module.exit_json(changed=changed, groups=user_groups, old_user_name=orig_name, new_user_name=new_name, old_path=path, new_path=new_path, keys=key_list, created_keys=new_key, user_meta=user_meta)
            elif (new_name and (not new_path) and (not been_updated)):
                module.exit_json(changed=changed, groups=user_groups, old_user_name=orig_name, new_user_name=new_name, keys=key_list, created_keys=new_key, user_meta=user_meta)
            elif (new_name and (not new_path) and been_updated):
                module.exit_json(changed=changed, groups=user_groups, user_name=new_name, keys=key_list, key_state=key_state, created_keys=new_key, user_meta=user_meta)
            elif ((not new_name) and new_path):
                module.exit_json(changed=changed, groups=user_groups, user_name=name, old_path=path, new_path=new_path, keys=key_list, created_keys=new_key, user_meta=user_meta)
            else:
                module.exit_json(changed=changed, groups=user_groups, user_name=name, keys=key_list, created_keys=new_key, user_meta=user_meta)
